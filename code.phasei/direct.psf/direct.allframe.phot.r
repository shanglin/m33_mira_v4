...
close(f.ask)
fs.as.master = c('ii6308.fits','ii6308.fits','ii6308.fits','ii6308.fits',
    'ii1009.fits', 'ii1009.fits', 'ii1009.fits', 'ii1009.fits',
    'ff1472.fits', 'gg1482.fits', 'gg1473.fits')
idx = which(fields == field)
if (sum(fields == field) != 1) stop(' No such field.')
f.as.master = fs.as.master[idx]

dir = paste0('~/Work/m33_mira/phot/i/',field,'_allframe/')
f.bad = 'bad.reg'
lf.bad = paste0(dir, f.bad)
if (!file.exists(lf.bad)) stop(' Please make a bad.reg file to indicate the mask')
...
write.photo.opt(dir, c(5,6,7,8,9,10,11,12,13))
write.allstar.opt(dir)
n.frames = length(list.files(dir, pattern = '.......fits'))
