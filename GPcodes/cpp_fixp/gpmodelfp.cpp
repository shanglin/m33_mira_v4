...
protected:
  ...
  double gp_m, // mean magnitude of the star
    sigmamSq, // variance of the mean magnitude. It will be an experiential constant, say, 2
    sigmabSq; // variance of the periodic component. It will be also an experiential constant, 2
  ...
  mat kernel_K, // Used to record the results of the square exponential kernel
...
public:
  vec MJD, // JD - 2450000, or what ever method to record time. By default they are double precision.
    mag, // magnitudes
...
void GP_Model::compute_CovM(vec & t, int k) {
  ...
  flag_inv = chol(gp_Ky_chol, gp_Ky); // v1
  // flag_inv = chol(gp_Ky_chol, gp_Ky, "lower"); // v2
  if (flag_inv) {
    //v1
    ...
    // //v1
    // gp_Ky_logdet = 2 * sum(log(gp_Ky_chol.diag())); // v2
    // alpha = solve(gp_Ky_chol.t(), solve(gp_Ky_chol, gp_y)); // v2
  }
}
...
void GP_Model::freq_est(vec all_freqs, int restart_sep) {
  ...
  bool restart;
  int i, j, k,
    n_trial_theta = 30; // Yes, 26 do not work for some lc
...
void GP_Model::load_input(string f_lc, string delimiter) {
  ...
  //[[2.5]] A simple sigma clip to remove outliers
