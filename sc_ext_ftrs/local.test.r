...
fig.dir = '~/Work/m33_phaseII/sc_ext_ftrs/local.test/figs/'
lc.dir = '~/Work/m33_phaseII/m33_v4_lcs/'
lst.dir = '~/Work/m33_phaseII/sc_ext_ftrs/local.test/'
spc.dir = '~/Work/m33_phaseII/sc_get_spec/part1.res/gp_spectra/'
out.dir = '~/Work/m33_phaseII/sc_ext_ftrs/local.test/features/'
...
do.extract = function(i) {
    ...
    ## ts = paste(f.lc, round(F.true,6), round(F.peak,6), round(Q.peak,3), round(Q.base,3), round(dQ.p1.base,3),
            ## round(dQ.p1.p2,3), round(theta.1,5), round(theta.2,5), round(A.model,3), round(A.lc,3), round(A.lc.9,3),
            ## n.obs, round(sd.error,3), round(posterior.sigma.a,3), round(posterior.sigma.b,3), posterior.mean.mag, sep='  ')
    log.theta.2 = log10(theta.2)
    if (!is.finite(log.theta.2)) log.theta.2 = 0
...
nfs.lc = length(fs.lc)
fs.lc = fs.lc[sample(1:nfs.lc)]

for (i in 30:100) {
    print(i)
    try(do.extract(i), silent=F)
}
