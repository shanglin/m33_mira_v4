...
cat(' Please input the field (i.e. w0i): ')
f.ask = file('stdin')
...
ifield = paste0('m0',substr(field,2,2),'i')
vfield = paste0('m0',substr(field,2,2),'v')
...
f.idmap = paste0(mdir,'m33masseyI.cat')
f.als = list.files(mdir, pattern=paste0('^M33.*.',ifield,'.als'))
if (length(f.als) != 1) {
    f.als = list.files(mdir, pattern=paste0('^M33.*.',vfield,'.als'))
    print('Using V band catalog as WCS reference')
...
print(paste0(' Field ',field,' finished.'))
